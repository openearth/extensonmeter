...
from orm_timeseries_nobv import Base
...
def resetindex(engine,schema):
    strsql = """SELECT tablename, indexname, indexdef FROM pg_indexes WHERE schemaname = 'nobv_timeseries' ORDER BY tablename,indexname;"""
    lst = engine.execute(strsql)
    for i in lst:
        print(i.tablename,i.indexname)
        strSql = """ALTER SEQUENCE nobv_timeseries.{indx} RESTART WITH 1""".format(indx=i.indexname)
        engine.execute(strSql)
