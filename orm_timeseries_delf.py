...
class TimeStep(Base):
    ...
    __table_args__ = {'schema': 'delftimeseries'}
    timestepkey    = Column(Integer,Sequence('delftimeseries.timesteps_timestepkey_seq'),primary_key=True)
...
class TimeSeries(Base):
    ...
    __table_args__ = {'schema': 'delftimeseries'}
    timeserieskey          = Column(Integer, Sequence('delftimeseries.timeserieskeys_timeserieskey_seq'),primary_key=True)
    locationkey            = Column(Integer, ForeignKey('delftimeseries.location.locationkey', ondelete='CASCADE'))
    parameterkey           = Column(Integer, ForeignKey('delftimeseries.parameter.parameterkey', ondelete='CASCADE'))
    timestepkey            = Column(Integer, ForeignKey('delftimeseries.timesteps.timestepkey', ondelete='CASCADE'))
    filesourcekey          = Column(Integer, ForeignKey('delftimeseries.filesource.filesourcekey', ondelete='CASCADE'))
...
class TimeSeriesComments(Base):
    ...
    __table_args__ = {'schema': 'delftimeseries'}
    timeserieskey  = Column(Integer,ForeignKey('delftimeseries.timeseries.timeserieskey', ondelete='CASCADE'),primary_key=True)
...
class Users(Base):
    __tablename__  = 'users'
    __table_args__ = {'schema': 'delftimeseries'}
    userkey        = Column(Integer,Sequence('delftimeseries.users_userkey_seq'),primary_key=True)
    id             = Column(String, unique=True,nullable=False)
    name           = Column(String)
...
class Flags(Base):
    __tablename__  = 'flags'
    __table_args__ = {'schema': 'delftimeseries'}
    flagkey        = Column(Integer,Sequence('delftimeseries.users_flagkey_seq'),primary_key=True)
    id             = Column(String, unique=True,nullable=False)
    name           = Column(String)    
...
class TimeSeriesManualEditsHistory(Base):
    ...
    datetime       = Column(DateTime, primary_key = True)
    userkey        = Column(Integer,ForeignKey('delftimeseries.users.userkey', ondelete='CASCADE'))
...
class TimeSeriesValuesAndFlags(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint('timeserieskey', 'datetime','scalarvalue'),{'schema': 'delftimeseries'})
    timeserieskey  = Column(Integer, ForeignKey('delftimeseries.timeseries.timeserieskey', ondelete='CASCADE'), nullable = False)
    datetime       = Column(DateTime, nullable=False)
    scalarvalue    = Column(Float,nullable=False)
    flags          = Column(Integer,ForeignKey('delftimeseries.flags.flagkey', ondelete='CASCADE'), nullable=False)

"""records pump history"""
class Parameter(Base):
    __tablename__      = 'parameter'
    __table_args__     = {'schema': 'delftimeseries'}
    parameterkey       = Column(Integer,Sequence('delftimeseries.parameter_parameterkey_seq'),primary_key=True)
    id                 = Column(String)    
    name               = Column(String)
    unitkey            = Column(Integer, ForeignKey('delftimeseries.unit.unitkey', ondelete='CASCADE'))
...
class Unit(Base):
    __tablename__      = 'unit'
    __table_args__     = {'schema': 'delftimeseries'}    
    unitkey            = Column(Integer,Sequence('delftimeseries.unitdesc_seq'),primary_key=True)
...
class FileSource(Base):
    ...
    __table_args__ = {'schema': 'delftimeseries'}    
    filesourcekey  = Column(Integer,Sequence('delftimeseries.filesource_seq'),primary_key=True)
...
class Transaction(Base):
    ...
    __table_args__  = {'schema': 'delftimeseries'}    
    transactionkey  = Column(Integer,Sequence('delftimeseries.transaction_seq'),primary_key=True)
    timeserieskey   = Column(Integer, ForeignKey('delftimeseries.timeseries.timeserieskey', ondelete='CASCADE'), nullable = False)
...
class Location(Base):
    ...
    __table_args__ = {'schema': 'delftimeseries'}
    locationkey        = Column(Integer,Sequence('delftimeseries.location_locationkey_seq1'),primary_key=True)    #key
    filesourcekey      = Column(Integer,ForeignKey('delftimeseries.filesource.filesourcekey', ondelete='CASCADE'))  #fskey
