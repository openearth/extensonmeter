...
pkeygws = sparameter(
    fc, "cm", "Grondwaterstand", ["cm-NAP", "centimeter tov NAP"], "Grondwaterstand"
)
pkeytempw = sparameter(
    fc, "Tw", "Temperatuur water", ["Celcius", "Celcius"], "Temperatuur water"
)
pkeytempi = sparameter(
    fc, "Ti", "Temperatuur intern", ["Celcius", "Celcius"], "Temperatuur intern"
)
...
df["diverid"] = df["diverid"].astype(
    "Int64"
)  # convert to Int64 to set data type as int array which can contain null values
...
if lid != df["locationkey"].iloc[-1]:
    ...
    subset.to_sql(
        "location", engine, schema="timeseries", index=None, if_exists="append"
    )
...
stmt = """SELECT locationkey, diverid from {s}.{t};""".format(
    s="timeseries", t="location"
)
...
for dir in lstdir:
    rmpath = "./{rm}/".format(
        rm=dir
    )  # remote path is link to the sftp and name of the folder
    sftp.listdir_attr(rmpath)
    for i in sftp.listdir_attr(rmpath):
        filepath = (lpath + "\\{rm}\\").format(
            rm=dir
        ) + i.filename  # assigning the right filename to file
        sftp.download(
            rmpath + i.filename, filepath
        )  # download from ftp to local location
        ...
        if (
            ...
            or name == "22072101"
            or name == "22091406"
        ):  # skip these ellitrack number as it is not correctly in metadata file yet
            continue

        locationkey = list(
            metadata.loc[metadata["diverid"] == name, "locationkey"]
        )  # find locationkey based on matadata table #name is folder name?
        print(name, locationkey)

        # administrate final store location of file source
        fskey = loadfilesource(
            (os.path.join(ppath, dir) + "\\" + i.filename), fc, "zoetwaterstijgghoogte"
        )
        ...
        skeygws = sserieskey(
            fc, pkeygws, locationkey[0], fskey[0], timestep="nonequidistant"
        )  # pkeygws
        skeytempw = sserieskey(
            fc, pkeytempw, locationkey[0], fskey[0], timestep="nonequidistant"
        )  # pkeytemp
        skeytempi = sserieskey(
            fc, pkeytempi, locationkey[0], fskey[0], timestep="nonequidistant"
        )  # pkeytemp
        ...
        if r != (dfw["datetime"].iloc[-1]).replace(tzinfo=None):
            dfw = df[["datetime", "Waterstand"]]
            dfw = dfw.rename(
                columns={"Waterstand": "scalarvalue"}
            )  # change column name
        ...
        if tempw != (dft["datetime"].iloc[-1]).replace(tzinfo=None):
            dft = df[["datetime", "Temperatuur water"]]
            dft = dft.rename(
                columns={"Temperatuur water": "scalarvalue"}
            )  # change column name
        ...
        if tempi != (dft["datetime"].iloc[-1]).replace(tzinfo=None):
            dfti = df[["datetime", "Temperatuur intern"]]
            dfti = dfti.rename(
                columns={"Temperatuur intern": "scalarvalue"}
            )  # change column name
        ...
        shutil.move(
            filepath, (os.path.join(ppath, dir) + "\\" + i.filename)
        )  # write to p drive
