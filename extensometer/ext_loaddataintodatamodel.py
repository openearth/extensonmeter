...
def update_location(sf):
    ...
    df = pd.read_excel(
        sf, names=["name", "diverid", "x", "y", "epsgcode"]
    )  #'tubetop','depth_pb','diverid', 'cablelength'])#,'filterid'])

    df["name"] = df["name"].str.lower()
    # find location or add to location table
    # before inserting, get latest id
    stmt = """SELECT max(locationkey) from {s}.{t};""".format(
    ...
    if lid != df["locationkey"].iloc[-1]:
        ...
        engine.execute(stmt)
        stmt = """select geom, locationkey, name, diverid from {s}.{t} """.format(
            s="timeseries", t="location"
        )
...
def metadata_location():
    ...
    stmt = """SELECT locationkey, name from {s}.{t};""".format(
        s="timeseries", t="location"
    )
...
def timeseries_todb(file, metadata, name):
    ...
    """
    locationkey = list(
        metadata.loc[metadata["name"] == name, "locationkey"]
