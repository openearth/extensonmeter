...
class TimeStep(Base):
    ...
    timestepkey    = Column(Integer,Sequence('timeseries.timesteps_timestepkey_seq'),primary_key=True)
...
class TimeSeries(Base):
    ...
    __table_args__ = {'schema': 'timeseries'}
    timeserieskey          = Column(Integer, Sequence('timeseries.timeserieskeys_timeserieskey_seq'),primary_key=True)
    locationkey            = Column(Integer, ForeignKey('timeseries.location.locationkey', ondelete='CASCADE'))
    parameterkey           = Column(Integer, ForeignKey('timeseries.parameter.parameterkey', ondelete='CASCADE'))
    timestepkey            = Column(Integer, ForeignKey('timeseries.timesteps.timestepkey', ondelete='CASCADE'))
    filesourcekey          = Column(Integer, ForeignKey('timeseries.filesource.filesourcekey', ondelete='CASCADE'))
...
class TimeSeriesComments(Base):
    ...
    __table_args__ = {'schema': 'timeseries'}
    timeserieskey  = Column(Integer,ForeignKey('timeseries.timeseries.timeserieskey', ondelete='CASCADE'),primary_key=True)
...
class Users(Base):
    __tablename__  = 'users'
    __table_args__ = {'schema': 'timeseries'}
    userkey        = Column(Integer,Sequence('timeseries.users_userkey_seq'),primary_key=True)
    id             = Column(String, unique=True,nullable=False)
    name           = Column(String)
...
class Flags(Base):
    __tablename__  = 'flags'
    __table_args__ = {'schema': 'timeseries'}
    flagkey        = Column(Integer,Sequence('timeseries.users_flagkey_seq'),primary_key=True)
    id             = Column(String, unique=True,nullable=False)
    name           = Column(String)    
...
class TimeSeriesManualEditsHistory(Base):
    ...
    datetime       = Column(DateTime, primary_key = True)
    userkey        = Column(Integer,ForeignKey('timeseries.users.userkey', ondelete='CASCADE'))
...
class TimeSeriesValuesAndFlags(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint('timeserieskey', 'datetime','scalarvalue'),{'schema': 'timeseries'})
    timeserieskey  = Column(Integer, ForeignKey('timeseries.timeseries.timeserieskey', ondelete='CASCADE'), nullable = False)
    datetime       = Column(DateTime, nullable=False)
    scalarvalue    = Column(Float,nullable=False)
    flags          = Column(Integer,ForeignKey('timeseries.flags.flagkey', ondelete='CASCADE'), nullable=False)

"""records pump history"""
class Parameter(Base):
    __tablename__      = 'parameter'
    __table_args__     = {'schema': 'timeseries'}
    parameterkey       = Column(Integer,Sequence('timeseries.parameter_parameterkey_seq'),primary_key=True)
    id                 = Column(String)    
    name               = Column(String)
...
class Transaction(Base):
    ...
    __table_args__  = {'schema': 'timeseries'}    
    transactionkey  = Column(Integer,Sequence('timeseries.transaction_seq'),primary_key=True)
    timeserieskey   = Column(Integer, ForeignKey('timeseries.timeseries.timeserieskey', ondelete='CASCADE'), nullable = False)
...
class Location(Base):
    ...
    __table_args__ = {'schema': 'timeseries'}
    locationkey        = Column(Integer,Sequence('timeseries.location_locationkey_seq1'),primary_key=True)    #key
    filesourcekey      = Column(Integer,ForeignKey('timeseries.filesource.filesourcekey', ondelete='CASCADE'))  #fskey
