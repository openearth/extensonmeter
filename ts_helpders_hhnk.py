...
from orm_timeseries_hhnk import Base, FileSource, Location, Parameter, Unit, TimeSeries, TimeStep, Flags, Transaction
...
def location(fc,fskey,name,x,y,epsg,shortname='',description='',z=0,altitude_msl=0, diverid=None,tubebot=None,tubetop=None):
    ...
    try:
        if str(f) == 'None':
            ...
            session.commit()
            if epsg == 28992:
                strsql = 'update hhnktimeseries.location set geom = st_setsrid(st_point(x,y),epsgcode) where geom is null'
            else:
                strsql = 'update hhnktimeseries.location set geom = st_transform(st_setsrid(st_point(x,y),epsgcode),28992) where geom is null'
